...
void PlayerEntity::update(Engine& engine, uint32_t msElapsed)
{
    ...
    {
        auto projectileEntity = std::make_shared<ProjectileEntity>(shared_from_this(), ProjectileEntity::ProjectileProperties{}, m_position + (m_heading * m_playerProperties.radius_km), m_heading * m_velocity.length());
